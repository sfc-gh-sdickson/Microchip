...
LEFT JOIN RAW.DESIGN_ENGINEERS e ON c.customer_id = e.customer_id AND e.engineer_status = 'ACTIVE'
...
LEFT JOIN RAW.PRODUCTION_ORDERS po ON c.customer_id = po.customer_id
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_ENGINEER_ANALYTICS AS
SELECT
    e.engineer_id,
    e.customer_id,
    c.customer_name,
    ...
    e.created_at,
    e.updated_at
FROM RAW.DESIGN_ENGINEERS e
JOIN RAW.CUSTOMERS c ON e.customer_id = c.customer_id
LEFT JOIN RAW.DESIGN_WINS dw ON e.engineer_id = dw.engineer_id
LEFT JOIN RAW.PRODUCTION_ORDERS po ON e.engineer_id = po.engineer_id
LEFT JOIN RAW.SUPPORT_TICKETS st ON e.engineer_id = st.engineer_id
LEFT JOIN RAW.CERTIFICATIONS cert ON e.engineer_id = cert.engineer_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_WIN_ANALYTICS AS
SELECT
    dw.design_win_id,
    dw.customer_id,
    c.customer_name,
...
JOIN RAW.DESIGN_ENGINEERS e ON dw.engineer_id = e.engineer_id
JOIN RAW.PRODUCT_CATALOG p ON dw.product_id = p.product_id
LEFT JOIN RAW.DISTRIBUTORS d ON dw.distributor_id = d.distributor_id
LEFT JOIN RAW.PRODUCTION_ORDERS po ON dw.design_win_id = po.design_win_id
GROUP BY
...
LEFT JOIN RAW.DESIGN_WINS dw ON p.product_id = dw.product_id
...
LEFT JOIN RAW.DESIGN_WINS dw ON d.distributor_id = dw.distributor_id
LEFT JOIN RAW.PRODUCTION_ORDERS po ON d.distributor_id = po.distributor_id
LEFT JOIN RAW.ORDERS o ON d.distributor_id = o.distributor_id
GROUP BY
...
LEFT JOIN RAW.DESIGN_ENGINEERS e ON st.engineer_id = e.engineer_id
LEFT JOIN RAW.SUPPORT_ENGINEERS se ON st.assigned_engineer_id = se.support_engineer_id
...
LEFT JOIN RAW.DESIGN_ENGINEERS e ON qi.reported_by_engineer_id = e.engineer_id;
...
LEFT JOIN RAW.PRODUCT_CATALOG p ON o.product_id = p.product_id
LEFT JOIN RAW.DISTRIBUTORS d ON o.distributor_id = d.distributor_id;
...
SELECT
    ...
    cert.created_at,
    cert.updated_at
FROM RAW.CERTIFICATIONS cert
JOIN RAW.DESIGN_ENGINEERS e ON cert.engineer_id = e.engineer_id
JOIN RAW.CUSTOMERS c ON cert.customer_id = c.customer_id
LEFT JOIN RAW.CERTIFICATION_VERIFICATIONS v ON cert.certification_id = v.certification_id
GROUP BY
