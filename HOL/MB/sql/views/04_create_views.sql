...
-- Customer 360 View
-- ============================================================================
CREATE OR REPLACE VIEW V_CUSTOMER_360_MB AS
SELECT
    c.customer_id,
    c.customer_name,
    ...
    c.created_at,
    c.updated_at
FROM RAW_MB.CUSTOMERS_MB c
LEFT JOIN RAW_MB.DESIGN_ENGINEERS_MB e ON c.customer_id = e.customer_id AND e.engineer_status = 'ACTIVE'
LEFT JOIN RAW_MB.SUPPORT_CONTRACTS_MB sc ON c.customer_id = sc.customer_id
LEFT JOIN RAW_MB.ORDERS_MB o ON c.customer_id = o.customer_id
LEFT JOIN RAW_MB.SUPPORT_TICKETS_MB st ON c.customer_id = st.customer_id
LEFT JOIN RAW_MB.DESIGN_WINS_MB dw ON c.customer_id = dw.customer_id
LEFT JOIN RAW_MB.PRODUCTION_ORDERS_MB po ON c.customer_id = po.customer_id
LEFT JOIN RAW_MB.QUALITY_ISSUES_MB qi ON c.customer_id = qi.customer_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_ENGINEER_ANALYTICS_MB AS
SELECT
    e.engineer_id,
    e.customer_id,
    c.customer_name,
    ...
    e.created_at,
    e.updated_at
FROM RAW_MB.DESIGN_ENGINEERS_MB e
JOIN RAW_MB.CUSTOMERS_MB c ON e.customer_id = c.customer_id
LEFT JOIN RAW_MB.DESIGN_WINS_MB dw ON e.engineer_id = dw.engineer_id
LEFT JOIN RAW_MB.PRODUCTION_ORDERS_MB po ON e.engineer_id = po.engineer_id
LEFT JOIN RAW_MB.SUPPORT_TICKETS_MB st ON e.engineer_id = st.engineer_id
LEFT JOIN RAW_MB.CERTIFICATIONS_MB cert ON e.engineer_id = cert.engineer_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_WIN_ANALYTICS_MB AS
SELECT
    dw.design_win_id,
    dw.customer_id,
    c.customer_name,
    ...
    dw.created_at,
    dw.updated_at
FROM RAW_MB.DESIGN_WINS_MB dw
JOIN RAW_MB.CUSTOMERS_MB c ON dw.customer_id = c.customer_id
JOIN RAW_MB.DESIGN_ENGINEERS_MB e ON dw.engineer_id = e.engineer_id
JOIN RAW_MB.PRODUCT_CATALOG_MB p ON dw.product_id = p.product_id
LEFT JOIN RAW_MB.DISTRIBUTORS_MB d ON dw.distributor_id = d.distributor_id
LEFT JOIN RAW_MB.PRODUCTION_ORDERS_MB po ON dw.design_win_id = po.design_win_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_PRODUCT_PERFORMANCE_MB AS
SELECT
    p.product_id,
    p.product_name,
    p.sku,
    ...
    p.created_at,
    p.updated_at
FROM RAW_MB.PRODUCT_CATALOG_MB p
LEFT JOIN RAW_MB.DESIGN_WINS_MB dw ON p.product_id = dw.product_id
LEFT JOIN RAW_MB.ORDERS_MB o ON p.product_id = o.product_id
LEFT JOIN RAW_MB.SUPPORT_TICKETS_MB st ON p.product_id = st.product_id
LEFT JOIN RAW_MB.QUALITY_ISSUES_MB qi ON p.product_id = qi.product_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DISTRIBUTOR_PERFORMANCE_MB AS
SELECT
    d.distributor_id,
    d.distributor_name,
    d.distributor_type,
    ...
    d.created_at,
    d.updated_at
FROM RAW_MB.DISTRIBUTORS_MB d
LEFT JOIN RAW_MB.DESIGN_WINS_MB dw ON d.distributor_id = dw.distributor_id
LEFT JOIN RAW_MB.PRODUCTION_ORDERS_MB po ON d.distributor_id = po.distributor_id
LEFT JOIN RAW_MB.ORDERS_MB o ON d.distributor_id = o.distributor_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_SUPPORT_TICKET_ANALYTICS_MB AS
SELECT
    st.ticket_id,
    st.customer_id,
    c.customer_name,
    ...
    st.created_at,
    st.updated_at
FROM RAW_MB.SUPPORT_TICKETS_MB st
JOIN RAW_MB.CUSTOMERS_MB c ON st.customer_id = c.customer_id
LEFT JOIN RAW_MB.DESIGN_ENGINEERS_MB e ON st.engineer_id = e.engineer_id
LEFT JOIN RAW_MB.SUPPORT_ENGINEERS_MB se ON st.assigned_engineer_id = se.support_engineer_id
LEFT JOIN RAW_MB.PRODUCT_CATALOG_MB p ON st.product_id = p.product_id;

-- ============================================================================
-- Quality Issue Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_QUALITY_ISSUE_ANALYTICS_MB AS
SELECT
    qi.quality_issue_id,
    qi.customer_id,
    c.customer_name,
    ...
    qi.created_at,
    qi.updated_at
FROM RAW_MB.QUALITY_ISSUES_MB qi
JOIN RAW_MB.CUSTOMERS_MB c ON qi.customer_id = c.customer_id
LEFT JOIN RAW_MB.PRODUCT_CATALOG_MB p ON qi.product_id = p.product_id
LEFT JOIN RAW_MB.DESIGN_ENGINEERS_MB e ON qi.reported_by_engineer_id = e.engineer_id;

-- ============================================================================
-- Revenue Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_REVENUE_ANALYTICS_MB AS
SELECT
    o.order_id,
    o.customer_id,
    c.customer_name,
    ...
    o.created_at,
    o.updated_at
FROM RAW_MB.ORDERS_MB o
JOIN RAW_MB.CUSTOMERS_MB c ON o.customer_id = c.customer_id
LEFT JOIN RAW_MB.PRODUCT_CATALOG_MB p ON o.product_id = p.product_id
LEFT JOIN RAW_MB.DISTRIBUTORS_MB d ON o.distributor_id = d.distributor_id;

-- ============================================================================
-- Certification Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_CERTIFICATION_ANALYTICS_MB AS
SELECT
    ...
    cert.created_at,
    cert.updated_at
FROM RAW_MB.CERTIFICATIONS_MB cert
JOIN RAW_MB.DESIGN_ENGINEERS_MB e ON cert.engineer_id = e.engineer_id
JOIN RAW_MB.CUSTOMERS_MB c ON cert.customer_id = c.customer_id
LEFT JOIN RAW_MB.CERTIFICATION_VERIFICATIONS_MB v ON cert.certification_id = v.certification_id
GROUP BY
