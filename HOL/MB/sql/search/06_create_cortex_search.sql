...
-- ============================================================================
CREATE OR REPLACE TABLE SUPPORT_TRANSCRIPTS_MB (
    ...
    FOREIGN KEY (ticket_id) REFERENCES SUPPORT_TICKETS_MB(ticket_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_MB(customer_id),
    FOREIGN KEY (support_engineer_id) REFERENCES SUPPORT_ENGINEERS_MB(support_engineer_id)
);

-- ============================================================================
-- Step 2: Create table for application notes
-- ============================================================================
CREATE OR REPLACE TABLE APPLICATION_NOTES_MB (
...
-- ============================================================================
CREATE OR REPLACE TABLE QUALITY_INVESTIGATION_REPORTS_MB (
    ...
    created_at TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(),
    FOREIGN KEY (quality_issue_id) REFERENCES QUALITY_ISSUES_MB(quality_issue_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_MB(customer_id),
    FOREIGN KEY (product_id) REFERENCES PRODUCT_CATALOG_MB(product_id)
);

-- ============================================================================
-- Step 4: Enable change tracking (required for Cortex Search)
-- ============================================================================
ALTER TABLE SUPPORT_TRANSCRIPTS_MB SET CHANGE_TRACKING = TRUE;
ALTER TABLE APPLICATION_NOTES_MB SET CHANGE_TRACKING = TRUE;
ALTER TABLE QUALITY_INVESTIGATION_REPORTS_MB SET CHANGE_TRACKING = TRUE;

-- ============================================================================
-- Step 5: Generate sample support transcripts
-- ============================================================================
INSERT INTO SUPPORT_TRANSCRIPTS_MB
SELECT
...
LEFT JOIN RAW_MB.PRODUCT_CATALOG_MB pc ON st.product_id = pc.product_id
...
-- ============================================================================
INSERT INTO APPLICATION_NOTES_MB VALUES
...
void enter_standby_mode(void) {
    ...
    SCB->SCR |= SCB_SCR_SLEEPDEEP_Msk;
    PM->SLEEP.reg = PM_SLEEP_IDLE_MBANDBY;
...
ADC->EVCTRL.reg = ADC_EVCTRL_MBARTEI;
```
...
-- ============================================================================
INSERT INTO QUALITY_INVESTIGATION_REPORTS_MB
SELECT
...
JOIN RAW_MB.PRODUCT_CATALOG_MB p ON qi.product_id = p.product_id
...
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE SUPPORT_TRANSCRIPTS_SEARCH_MB
  ON transcript_text
...
AS
  SELECT
    ...
    resolution_provided,
    created_at
  FROM RAW_MB.SUPPORT_TRANSCRIPTS_MB;

-- ============================================================================
-- Step 9: Create Cortex Search Service for Application Notes
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE APPLICATION_NOTES_SEARCH_MB
  ON content
...
AS
  SELECT
    ...
    download_count,
    created_at
  FROM RAW_MB.APPLICATION_NOTES_MB;

-- ============================================================================
-- Step 10: Create Cortex Search Service for Quality Investigation Reports
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE QUALITY_INVESTIGATION_REPORTS_SEARCH_MB
  ON report_text
...
AS
  SELECT
    ...
    investigated_by,
    created_at
  FROM RAW_MB.QUALITY_INVESTIGATION_REPORTS_MB;

-- ============================================================================
-- Step 11: Verify Cortex Search Services Created
-- ============================================================================
SHOW CORTEX SEARCH SERVICES IN SCHEMA RAW_MB;
...
FROM (
  SELECT 'SUPPORT_TRANSCRIPTS_SEARCH_MB' AS service_name
  UNION ALL
  SELECT 'APPLICATION_NOTES_SEARCH_MB'
  UNION ALL
  SELECT 'QUALITY_INVESTIGATION_REPORTS_SEARCH_MB'
);

-- ============================================================================
-- Display data counts
-- ============================================================================
SELECT 'SUPPORT_TRANSCRIPTS_MB' AS table_name, COUNT(*) AS row_count FROM SUPPORT_TRANSCRIPTS_MB
UNION ALL
SELECT 'APPLICATION_NOTES_MB', COUNT(*) FROM APPLICATION_NOTES_MB
UNION ALL
SELECT 'QUALITY_INVESTIGATION_REPORTS_MB', COUNT(*) FROM QUALITY_INVESTIGATION_REPORTS_MB
ORDER BY table_name;
