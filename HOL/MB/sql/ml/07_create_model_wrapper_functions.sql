...
DROP FUNCTION IF EXISTS PREDICT_REVENUE_MB(INT);

CREATE OR REPLACE PROCEDURE PREDICT_REVENUE_MB(
    MONTHS_AHEAD INT
)
RETURNS STRING
LANGUAGE PYTHON
...
-- Drop if exists (in case it was created as FUNCTION before)
DROP FUNCTION IF EXISTS PREDICT_CUSTOMER_CHURN_MB(STRING);

CREATE OR REPLACE PROCEDURE PREDICT_CUSTOMER_CHURN_MB(
...
-- Drop if exists (in case it was created as FUNCTION before)
DROP FUNCTION IF EXISTS PREDICT_DESIGN_WIN_CONVERSION_MB(STRING);

CREATE OR REPLACE PROCEDURE PREDICT_DESIGN_WIN_CONVERSION_MB(
...
/*
CALL PREDICT_REVENUE_MB(6);
CALL PREDICT_CUSTOMER_CHURN_MB('OEM');
CALL PREDICT_DESIGN_WIN_CONVERSION_MB('PIC');
*/
