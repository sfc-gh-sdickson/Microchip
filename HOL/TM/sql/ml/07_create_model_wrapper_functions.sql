...
DROP FUNCTION IF EXISTS PREDICT_REVENUE_TM(INT);

CREATE OR REPLACE PROCEDURE PREDICT_REVENUE_TM(
    MONTHS_AHEAD INT
)
RETURNS STRING
LANGUAGE PYTHON
...
-- Drop if exists (in case it was created as FUNCTION before)
DROP FUNCTION IF EXISTS PREDICT_CUSTOMER_CHURN_TM(STRING);

CREATE OR REPLACE PROCEDURE PREDICT_CUSTOMER_CHURN_TM(
...
-- Drop if exists (in case it was created as FUNCTION before)
DROP FUNCTION IF EXISTS PREDICT_DESIGN_WIN_CONVERSION_TM(STRING);

CREATE OR REPLACE PROCEDURE PREDICT_DESIGN_WIN_CONVERSION_TM(
...
/*
CALL PREDICT_REVENUE_TM(6);
CALL PREDICT_CUSTOMER_CHURN_TM('OEM');
CALL PREDICT_DESIGN_WIN_CONVERSION_TM('PIC');
*/
