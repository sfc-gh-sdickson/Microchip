...
-- ============================================================================
CREATE OR REPLACE TABLE SUPPORT_TRANSCRIPTS_TM (
    ...
    FOREIGN KEY (ticket_id) REFERENCES SUPPORT_TICKETS_TM(ticket_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_TM(customer_id),
    FOREIGN KEY (support_engineer_id) REFERENCES SUPPORT_ENGINEERS_TM(support_engineer_id)
);

-- ============================================================================
-- Step 2: Create table for application notes
-- ============================================================================
CREATE OR REPLACE TABLE APPLICATION_NOTES_TM (
...
-- ============================================================================
CREATE OR REPLACE TABLE QUALITY_INVESTIGATION_REPORTS_TM (
    ...
    created_at TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(),
    FOREIGN KEY (quality_issue_id) REFERENCES QUALITY_ISSUES_TM(quality_issue_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_TM(customer_id),
    FOREIGN KEY (product_id) REFERENCES PRODUCT_CATALOG_TM(product_id)
);

-- ============================================================================
-- Step 4: Enable change tracking (required for Cortex Search)
-- ============================================================================
ALTER TABLE SUPPORT_TRANSCRIPTS_TM SET CHANGE_TRACKING = TRUE;
ALTER TABLE APPLICATION_NOTES_TM SET CHANGE_TRACKING = TRUE;
ALTER TABLE QUALITY_INVESTIGATION_REPORTS_TM SET CHANGE_TRACKING = TRUE;

-- ============================================================================
-- Step 5: Generate sample support transcripts
-- ============================================================================
INSERT INTO SUPPORT_TRANSCRIPTS_TM
SELECT
...
LEFT JOIN RAW_TM.PRODUCT_CATALOG_TM pc ON st.product_id = pc.product_id
...
-- ============================================================================
INSERT INTO APPLICATION_NOTES_TM VALUES
...
void enter_standby_mode(void) {
    ...
    SCB->SCR |= SCB_SCR_SLEEPDEEP_Msk;
    PM->SLEEP.reg = PM_SLEEP_IDLE_TMANDBY;
...
ADC->EVCTRL.reg = ADC_EVCTRL_TMARTEI;
```
...
-- ============================================================================
INSERT INTO QUALITY_INVESTIGATION_REPORTS_TM
SELECT
...
JOIN RAW_TM.PRODUCT_CATALOG_TM p ON qi.product_id = p.product_id
...
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE SUPPORT_TRANSCRIPTS_SEARCH_TM
  ON transcript_text
...
AS
  SELECT
    ...
    resolution_provided,
    created_at
  FROM RAW_TM.SUPPORT_TRANSCRIPTS_TM;

-- ============================================================================
-- Step 9: Create Cortex Search Service for Application Notes
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE APPLICATION_NOTES_SEARCH_TM
  ON content
...
AS
  SELECT
    ...
    download_count,
    created_at
  FROM RAW_TM.APPLICATION_NOTES_TM;

-- ============================================================================
-- Step 10: Create Cortex Search Service for Quality Investigation Reports
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE QUALITY_INVESTIGATION_REPORTS_SEARCH_TM
  ON report_text
...
AS
  SELECT
    ...
    investigated_by,
    created_at
  FROM RAW_TM.QUALITY_INVESTIGATION_REPORTS_TM;

-- ============================================================================
-- Step 11: Verify Cortex Search Services Created
-- ============================================================================
SHOW CORTEX SEARCH SERVICES IN SCHEMA RAW_TM;
...
FROM (
  SELECT 'SUPPORT_TRANSCRIPTS_SEARCH_TM' AS service_name
  UNION ALL
  SELECT 'APPLICATION_NOTES_SEARCH_TM'
  UNION ALL
  SELECT 'QUALITY_INVESTIGATION_REPORTS_SEARCH_TM'
);

-- ============================================================================
-- Display data counts
-- ============================================================================
SELECT 'SUPPORT_TRANSCRIPTS_TM' AS table_name, COUNT(*) AS row_count FROM SUPPORT_TRANSCRIPTS_TM
UNION ALL
SELECT 'APPLICATION_NOTES_TM', COUNT(*) FROM APPLICATION_NOTES_TM
UNION ALL
SELECT 'QUALITY_INVESTIGATION_REPORTS_TM', COUNT(*) FROM QUALITY_INVESTIGATION_REPORTS_TM
ORDER BY table_name;
