...
-- Customer 360 View
-- ============================================================================
CREATE OR REPLACE VIEW V_CUSTOMER_360_VP AS
SELECT
    c.customer_id,
    c.customer_name,
    ...
    c.created_at,
    c.updated_at
FROM RAW_VP.CUSTOMERS_VP c
LEFT JOIN RAW_VP.DESIGN_ENGINEERS_VP e ON c.customer_id = e.customer_id AND e.engineer_status = 'ACTIVE'
LEFT JOIN RAW_VP.SUPPORT_CONTRACTS_VP sc ON c.customer_id = sc.customer_id
LEFT JOIN RAW_VP.ORDERS_VP o ON c.customer_id = o.customer_id
LEFT JOIN RAW_VP.SUPPORT_TICKETS_VP st ON c.customer_id = st.customer_id
LEFT JOIN RAW_VP.DESIGN_WINS_VP dw ON c.customer_id = dw.customer_id
LEFT JOIN RAW_VP.PRODUCTION_ORDERS_VP po ON c.customer_id = po.customer_id
LEFT JOIN RAW_VP.QUALITY_ISSUES_VP qi ON c.customer_id = qi.customer_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_ENGINEER_ANALYTICS_VP AS
SELECT
    e.engineer_id,
    e.customer_id,
    c.customer_name,
    ...
    e.created_at,
    e.updated_at
FROM RAW_VP.DESIGN_ENGINEERS_VP e
JOIN RAW_VP.CUSTOMERS_VP c ON e.customer_id = c.customer_id
LEFT JOIN RAW_VP.DESIGN_WINS_VP dw ON e.engineer_id = dw.engineer_id
LEFT JOIN RAW_VP.PRODUCTION_ORDERS_VP po ON e.engineer_id = po.engineer_id
LEFT JOIN RAW_VP.SUPPORT_TICKETS_VP st ON e.engineer_id = st.engineer_id
LEFT JOIN RAW_VP.CERTIFICATIONS_VP cert ON e.engineer_id = cert.engineer_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_WIN_ANALYTICS_VP AS
SELECT
    dw.design_win_id,
    dw.customer_id,
    c.customer_name,
    ...
    dw.created_at,
    dw.updated_at
FROM RAW_VP.DESIGN_WINS_VP dw
JOIN RAW_VP.CUSTOMERS_VP c ON dw.customer_id = c.customer_id
JOIN RAW_VP.DESIGN_ENGINEERS_VP e ON dw.engineer_id = e.engineer_id
JOIN RAW_VP.PRODUCT_CATALOG_VP p ON dw.product_id = p.product_id
LEFT JOIN RAW_VP.DISTRIBUTORS_VP d ON dw.distributor_id = d.distributor_id
LEFT JOIN RAW_VP.PRODUCTION_ORDERS_VP po ON dw.design_win_id = po.design_win_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_PRODUCT_PERFORMANCE_VP AS
SELECT
    p.product_id,
    p.product_name,
    p.sku,
    ...
    p.created_at,
    p.updated_at
FROM RAW_VP.PRODUCT_CATALOG_VP p
LEFT JOIN RAW_VP.DESIGN_WINS_VP dw ON p.product_id = dw.product_id
LEFT JOIN RAW_VP.ORDERS_VP o ON p.product_id = o.product_id
LEFT JOIN RAW_VP.SUPPORT_TICKETS_VP st ON p.product_id = st.product_id
LEFT JOIN RAW_VP.QUALITY_ISSUES_VP qi ON p.product_id = qi.product_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DISTRIBUTOR_PERFORMANCE_VP AS
SELECT
    d.distributor_id,
    d.distributor_name,
    d.distributor_type,
    ...
    d.created_at,
    d.updated_at
FROM RAW_VP.DISTRIBUTORS_VP d
LEFT JOIN RAW_VP.DESIGN_WINS_VP dw ON d.distributor_id = dw.distributor_id
LEFT JOIN RAW_VP.PRODUCTION_ORDERS_VP po ON d.distributor_id = po.distributor_id
LEFT JOIN RAW_VP.ORDERS_VP o ON d.distributor_id = o.distributor_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_SUPPORT_TICKET_ANALYTICS_VP AS
SELECT
    st.ticket_id,
    st.customer_id,
    c.customer_name,
    ...
    st.created_at,
    st.updated_at
FROM RAW_VP.SUPPORT_TICKETS_VP st
JOIN RAW_VP.CUSTOMERS_VP c ON st.customer_id = c.customer_id
LEFT JOIN RAW_VP.DESIGN_ENGINEERS_VP e ON st.engineer_id = e.engineer_id
LEFT JOIN RAW_VP.SUPPORT_ENGINEERS_VP se ON st.assigned_engineer_id = se.support_engineer_id
LEFT JOIN RAW_VP.PRODUCT_CATALOG_VP p ON st.product_id = p.product_id;

-- ============================================================================
-- Quality Issue Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_QUALITY_ISSUE_ANALYTICS_VP AS
SELECT
    qi.quality_issue_id,
    qi.customer_id,
    c.customer_name,
    ...
    qi.created_at,
    qi.updated_at
FROM RAW_VP.QUALITY_ISSUES_VP qi
JOIN RAW_VP.CUSTOMERS_VP c ON qi.customer_id = c.customer_id
LEFT JOIN RAW_VP.PRODUCT_CATALOG_VP p ON qi.product_id = p.product_id
LEFT JOIN RAW_VP.DESIGN_ENGINEERS_VP e ON qi.reported_by_engineer_id = e.engineer_id;

-- ============================================================================
-- Revenue Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_REVENUE_ANALYTICS_VP AS
SELECT
    o.order_id,
    o.customer_id,
    c.customer_name,
    ...
    o.created_at,
    o.updated_at
FROM RAW_VP.ORDERS_VP o
JOIN RAW_VP.CUSTOMERS_VP c ON o.customer_id = c.customer_id
LEFT JOIN RAW_VP.PRODUCT_CATALOG_VP p ON o.product_id = p.product_id
LEFT JOIN RAW_VP.DISTRIBUTORS_VP d ON o.distributor_id = d.distributor_id;

-- ============================================================================
-- Certification Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_CERTIFICATION_ANALYTICS_VP AS
SELECT
    ...
    cert.created_at,
    cert.updated_at
FROM RAW_VP.CERTIFICATIONS_VP cert
JOIN RAW_VP.DESIGN_ENGINEERS_VP e ON cert.engineer_id = e.engineer_id
JOIN RAW_VP.CUSTOMERS_VP c ON cert.customer_id = c.customer_id
LEFT JOIN RAW_VP.CERTIFICATION_VERIFICATIONS_VP v ON cert.certification_id = v.certification_id
GROUP BY
