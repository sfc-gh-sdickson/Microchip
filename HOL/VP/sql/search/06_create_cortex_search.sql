...
-- ============================================================================
CREATE OR REPLACE TABLE SUPPORT_TRANSCRIPTS_VP (
    ...
    FOREIGN KEY (ticket_id) REFERENCES SUPPORT_TICKETS_VP(ticket_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_VP(customer_id),
    FOREIGN KEY (support_engineer_id) REFERENCES SUPPORT_ENGINEERS_VP(support_engineer_id)
);

-- ============================================================================
-- Step 2: Create table for application notes
-- ============================================================================
CREATE OR REPLACE TABLE APPLICATION_NOTES_VP (
...
-- ============================================================================
CREATE OR REPLACE TABLE QUALITY_INVESTIGATION_REPORTS_VP (
    ...
    created_at TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(),
    FOREIGN KEY (quality_issue_id) REFERENCES QUALITY_ISSUES_VP(quality_issue_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_VP(customer_id),
    FOREIGN KEY (product_id) REFERENCES PRODUCT_CATALOG_VP(product_id)
);

-- ============================================================================
-- Step 4: Enable change tracking (required for Cortex Search)
-- ============================================================================
ALTER TABLE SUPPORT_TRANSCRIPTS_VP SET CHANGE_TRACKING = TRUE;
ALTER TABLE APPLICATION_NOTES_VP SET CHANGE_TRACKING = TRUE;
ALTER TABLE QUALITY_INVESTIGATION_REPORTS_VP SET CHANGE_TRACKING = TRUE;

-- ============================================================================
-- Step 5: Generate sample support transcripts
-- ============================================================================
INSERT INTO SUPPORT_TRANSCRIPTS_VP
SELECT
...
LEFT JOIN RAW_VP.PRODUCT_CATALOG_VP pc ON st.product_id = pc.product_id
...
-- ============================================================================
INSERT INTO APPLICATION_NOTES_VP VALUES
...
void enter_standby_mode(void) {
    ...
    SCB->SCR |= SCB_SCR_SLEEPDEEP_Msk;
    PM->SLEEP.reg = PM_SLEEP_IDLE_VPANDBY;
...
ADC->EVCTRL.reg = ADC_EVCTRL_VPARTEI;
```
...
-- ============================================================================
INSERT INTO QUALITY_INVESTIGATION_REPORTS_VP
SELECT
...
JOIN RAW_VP.PRODUCT_CATALOG_VP p ON qi.product_id = p.product_id
...
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE SUPPORT_TRANSCRIPTS_SEARCH_VP
  ON transcript_text
...
AS
  SELECT
    ...
    resolution_provided,
    created_at
  FROM RAW_VP.SUPPORT_TRANSCRIPTS_VP;

-- ============================================================================
-- Step 9: Create Cortex Search Service for Application Notes
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE APPLICATION_NOTES_SEARCH_VP
  ON content
...
AS
  SELECT
    ...
    download_count,
    created_at
  FROM RAW_VP.APPLICATION_NOTES_VP;

-- ============================================================================
-- Step 10: Create Cortex Search Service for Quality Investigation Reports
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE QUALITY_INVESTIGATION_REPORTS_SEARCH_VP
  ON report_text
...
AS
  SELECT
    ...
    investigated_by,
    created_at
  FROM RAW_VP.QUALITY_INVESTIGATION_REPORTS_VP;

-- ============================================================================
-- Step 11: Verify Cortex Search Services Created
-- ============================================================================
SHOW CORTEX SEARCH SERVICES IN SCHEMA RAW_VP;
...
FROM (
  SELECT 'SUPPORT_TRANSCRIPTS_SEARCH_VP' AS service_name
  UNION ALL
  SELECT 'APPLICATION_NOTES_SEARCH_VP'
  UNION ALL
  SELECT 'QUALITY_INVESTIGATION_REPORTS_SEARCH_VP'
);

-- ============================================================================
-- Display data counts
-- ============================================================================
SELECT 'SUPPORT_TRANSCRIPTS_VP' AS table_name, COUNT(*) AS row_count FROM SUPPORT_TRANSCRIPTS_VP
UNION ALL
SELECT 'APPLICATION_NOTES_VP', COUNT(*) FROM APPLICATION_NOTES_VP
UNION ALL
SELECT 'QUALITY_INVESTIGATION_REPORTS_VP', COUNT(*) FROM QUALITY_INVESTIGATION_REPORTS_VP
ORDER BY table_name;
