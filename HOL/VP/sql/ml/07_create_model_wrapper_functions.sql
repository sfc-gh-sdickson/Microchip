...
DROP FUNCTION IF EXISTS PREDICT_REVENUE_VP(INT);

CREATE OR REPLACE PROCEDURE PREDICT_REVENUE_VP(
    MONTHS_AHEAD INT
)
RETURNS STRING
LANGUAGE PYTHON
...
-- Drop if exists (in case it was created as FUNCTION before)
DROP FUNCTION IF EXISTS PREDICT_CUSTOMER_CHURN_VP(STRING);

CREATE OR REPLACE PROCEDURE PREDICT_CUSTOMER_CHURN_VP(
...
-- Drop if exists (in case it was created as FUNCTION before)
DROP FUNCTION IF EXISTS PREDICT_DESIGN_WIN_CONVERSION_VP(STRING);

CREATE OR REPLACE PROCEDURE PREDICT_DESIGN_WIN_CONVERSION_VP(
...
/*
CALL PREDICT_REVENUE_VP(6);
CALL PREDICT_CUSTOMER_CHURN_VP('OEM');
CALL PREDICT_DESIGN_WIN_CONVERSION_VP('PIC');
*/
