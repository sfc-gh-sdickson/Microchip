...
-- ============================================================================
CREATE OR REPLACE TABLE SUPPORT_TRANSCRIPTS_RN (
    ...
    FOREIGN KEY (ticket_id) REFERENCES SUPPORT_TICKETS_RN(ticket_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_RN(customer_id),
    FOREIGN KEY (support_engineer_id) REFERENCES SUPPORT_ENGINEERS_RN(support_engineer_id)
);

-- ============================================================================
-- Step 2: Create table for application notes
-- ============================================================================
CREATE OR REPLACE TABLE APPLICATION_NOTES_RN (
...
-- ============================================================================
CREATE OR REPLACE TABLE QUALITY_INVESTIGATION_REPORTS_RN (
    ...
    created_at TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(),
    FOREIGN KEY (quality_issue_id) REFERENCES QUALITY_ISSUES_RN(quality_issue_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_RN(customer_id),
...
-- ============================================================================
ALTER TABLE SUPPORT_TRANSCRIPTS_RN SET CHANGE_TRACKING = TRUE;
ALTER TABLE APPLICATION_NOTES_RN SET CHANGE_TRACKING = TRUE;
ALTER TABLE QUALITY_INVESTIGATION_REPORTS_RN SET CHANGE_TRACKING = TRUE;

-- ============================================================================
-- Step 5: Generate sample support transcripts
-- ============================================================================
INSERT INTO SUPPORT_TRANSCRIPTS_RN
SELECT
...
LEFT JOIN RAW_RN.PRODUCT_CATALOG_RN pc ON st.product_id = pc.product_id
...
-- ============================================================================
INSERT INTO APPLICATION_NOTES_RN VALUES
...
void enter_standby_mode(void) {
    ...
    SCB->SCR |= SCB_SCR_SLEEPDEEP_Msk;
    PM->SLEEP.reg = PM_SLEEP_IDLE_RNANDBY;
...
ADC->EVCTRL.reg = ADC_EVCTRL_RNARTEI;
```
...
-- ============================================================================
INSERT INTO QUALITY_INVESTIGATION_REPORTS_RN
SELECT
...
JOIN RAW_RN.PRODUCT_CATALOG_RN p ON qi.product_id = p.product_id
...
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE SUPPORT_TRANSCRIPTS_SEARCH_RN
  ON transcript_text
...
AS
  SELECT
    ...
    resolution_provided,
    created_at
  FROM RAW_RN.SUPPORT_TRANSCRIPTS_RN;

-- ============================================================================
-- Step 9: Create Cortex Search Service for Application Notes
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE APPLICATION_NOTES_SEARCH_RN
  ON content
...
AS
  SELECT
    ...
    download_count,
    created_at
  FROM RAW_RN.APPLICATION_NOTES_RN;

-- ============================================================================
-- Step 10: Create Cortex Search Service for Quality Investigation Reports
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE QUALITY_INVESTIGATION_REPORTS_SEARCH_RN
  ON report_text
...
AS
  SELECT
    ...
    investigated_by,
    created_at
  FROM RAW_RN.QUALITY_INVESTIGATION_REPORTS_RN;

-- ============================================================================
-- Step 11: Verify Cortex Search Services Created
-- ============================================================================
SHOW CORTEX SEARCH SERVICES IN SCHEMA RAW_RN;
...
FROM (
  SELECT 'SUPPORT_TRANSCRIPTS_SEARCH_RN' AS service_name
  UNION ALL
  SELECT 'APPLICATION_NOTES_SEARCH_RN'
  UNION ALL
  SELECT 'QUALITY_INVESTIGATION_REPORTS_SEARCH_RN'
);

-- ============================================================================
-- Display data counts
-- ============================================================================
SELECT 'SUPPORT_TRANSCRIPTS_RN' AS table_name, COUNT(*) AS row_count FROM SUPPORT_TRANSCRIPTS_RN
UNION ALL
SELECT 'APPLICATION_NOTES_RN', COUNT(*) FROM APPLICATION_NOTES_RN
UNION ALL
SELECT 'QUALITY_INVESTIGATION_REPORTS_RN', COUNT(*) FROM QUALITY_INVESTIGATION_REPORTS_RN
ORDER BY table_name;
