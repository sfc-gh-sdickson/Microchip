...
-- Customer 360 View
-- ============================================================================
CREATE OR REPLACE VIEW V_CUSTOMER_360_RN AS
SELECT
    c.customer_id,
    c.customer_name,
    ...
    c.created_at,
    c.updated_at
FROM RAW_RN.CUSTOMERS_RN c
LEFT JOIN RAW_RN.DESIGN_ENGINEERS_RN e ON c.customer_id = e.customer_id AND e.engineer_status = 'ACTIVE'
LEFT JOIN RAW_RN.SUPPORT_CONTRACTS_RN sc ON c.customer_id = sc.customer_id
LEFT JOIN RAW_RN.ORDERS_RN o ON c.customer_id = o.customer_id
LEFT JOIN RAW_RN.SUPPORT_TICKETS_RN st ON c.customer_id = st.customer_id
LEFT JOIN RAW_RN.DESIGN_WINS_RN dw ON c.customer_id = dw.customer_id
LEFT JOIN RAW_RN.PRODUCTION_ORDERS_RN po ON c.customer_id = po.customer_id
LEFT JOIN RAW_RN.QUALITY_ISSUES_RN qi ON c.customer_id = qi.customer_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_ENGINEER_ANALYTICS_RN AS
SELECT
    e.engineer_id,
    e.customer_id,
    c.customer_name,
    ...
    e.created_at,
    e.updated_at
FROM RAW_RN.DESIGN_ENGINEERS_RN e
JOIN RAW_RN.CUSTOMERS_RN c ON e.customer_id = c.customer_id
LEFT JOIN RAW_RN.DESIGN_WINS_RN dw ON e.engineer_id = dw.engineer_id
LEFT JOIN RAW_RN.PRODUCTION_ORDERS_RN po ON e.engineer_id = po.engineer_id
LEFT JOIN RAW_RN.SUPPORT_TICKETS_RN st ON e.engineer_id = st.engineer_id
LEFT JOIN RAW_RN.CERTIFICATIONS_RN cert ON e.engineer_id = cert.engineer_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_WIN_ANALYTICS_RN AS
SELECT
    dw.design_win_id,
    dw.customer_id,
    c.customer_name,
    ...
    dw.created_at,
    dw.updated_at
FROM RAW_RN.DESIGN_WINS_RN dw
JOIN RAW_RN.CUSTOMERS_RN c ON dw.customer_id = c.customer_id
JOIN RAW_RN.DESIGN_ENGINEERS_RN e ON dw.engineer_id = e.engineer_id
JOIN RAW_RN.PRODUCT_CATALOG_RN p ON dw.product_id = p.product_id
LEFT JOIN RAW_RN.DISTRIBUTORS_RN d ON dw.distributor_id = d.distributor_id
LEFT JOIN RAW_RN.PRODUCTION_ORDERS_RN po ON dw.design_win_id = po.design_win_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_PRODUCT_PERFORMANCE_RN AS
SELECT
    p.product_id,
    p.product_name,
    p.sku,
    ...
    p.created_at,
    p.updated_at
FROM RAW_RN.PRODUCT_CATALOG_RN p
LEFT JOIN RAW_RN.DESIGN_WINS_RN dw ON p.product_id = dw.product_id
LEFT JOIN RAW_RN.ORDERS_RN o ON p.product_id = o.product_id
LEFT JOIN RAW_RN.SUPPORT_TICKETS_RN st ON p.product_id = st.product_id
LEFT JOIN RAW_RN.QUALITY_ISSUES_RN qi ON p.product_id = qi.product_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DISTRIBUTOR_PERFORMANCE_RN AS
SELECT
    d.distributor_id,
    d.distributor_name,
    d.distributor_type,
    ...
    d.created_at,
    d.updated_at
FROM RAW_RN.DISTRIBUTORS_RN d
LEFT JOIN RAW_RN.DESIGN_WINS_RN dw ON d.distributor_id = dw.distributor_id
LEFT JOIN RAW_RN.PRODUCTION_ORDERS_RN po ON d.distributor_id = po.distributor_id
LEFT JOIN RAW_RN.ORDERS_RN o ON d.distributor_id = o.distributor_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_SUPPORT_TICKET_ANALYTICS_RN AS
SELECT
    st.ticket_id,
    st.customer_id,
    c.customer_name,
    ...
    st.created_at,
    st.updated_at
FROM RAW_RN.SUPPORT_TICKETS_RN st
JOIN RAW_RN.CUSTOMERS_RN c ON st.customer_id = c.customer_id
LEFT JOIN RAW_RN.DESIGN_ENGINEERS_RN e ON st.engineer_id = e.engineer_id
LEFT JOIN RAW_RN.SUPPORT_ENGINEERS_RN se ON st.assigned_engineer_id = se.support_engineer_id
LEFT JOIN RAW_RN.PRODUCT_CATALOG_RN p ON st.product_id = p.product_id;

-- ============================================================================
-- Quality Issue Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_QUALITY_ISSUE_ANALYTICS_RN AS
SELECT
    qi.quality_issue_id,
    qi.customer_id,
    c.customer_name,
    ...
    qi.created_at,
    qi.updated_at
FROM RAW_RN.QUALITY_ISSUES_RN qi
JOIN RAW_RN.CUSTOMERS_RN c ON qi.customer_id = c.customer_id
LEFT JOIN RAW_RN.PRODUCT_CATALOG_RN p ON qi.product_id = p.product_id
LEFT JOIN RAW_RN.DESIGN_ENGINEERS_RN e ON qi.reported_by_engineer_id = e.engineer_id;

-- ============================================================================
-- Revenue Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_REVENUE_ANALYTICS_RN AS
SELECT
    o.order_id,
    o.customer_id,
    c.customer_name,
    ...
    o.created_at,
    o.updated_at
FROM RAW_RN.ORDERS_RN o
JOIN RAW_RN.CUSTOMERS_RN c ON o.customer_id = c.customer_id
LEFT JOIN RAW_RN.PRODUCT_CATALOG_RN p ON o.product_id = p.product_id
LEFT JOIN RAW_RN.DISTRIBUTORS_RN d ON o.distributor_id = d.distributor_id;

-- ============================================================================
-- Certification Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_CERTIFICATION_ANALYTICS_RN AS
SELECT
    ...
    cert.created_at,
    cert.updated_at
FROM RAW_RN.CERTIFICATIONS_RN cert
JOIN RAW_RN.DESIGN_ENGINEERS_RN e ON cert.engineer_id = e.engineer_id
JOIN RAW_RN.CUSTOMERS_RN c ON cert.customer_id = c.customer_id
LEFT JOIN RAW_RN.CERTIFICATION_VERIFICATIONS_RN v ON cert.certification_id = v.certification_id
GROUP BY
