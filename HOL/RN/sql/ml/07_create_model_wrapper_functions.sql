...
DROP FUNCTION IF EXISTS PREDICT_REVENUE_RN(INT);

CREATE OR REPLACE PROCEDURE PREDICT_REVENUE_RN(
    MONTHS_AHEAD INT
)
RETURNS STRING
LANGUAGE PYTHON
...
-- Drop if exists (in case it was created as FUNCTION before)
DROP FUNCTION IF EXISTS PREDICT_CUSTOMER_CHURN_RN(STRING);

CREATE OR REPLACE PROCEDURE PREDICT_CUSTOMER_CHURN_RN(
...
-- Drop if exists (in case it was created as FUNCTION before)
DROP FUNCTION IF EXISTS PREDICT_DESIGN_WIN_CONVERSION_RN(STRING);

CREATE OR REPLACE PROCEDURE PREDICT_DESIGN_WIN_CONVERSION_RN(
...
/*
CALL PREDICT_REVENUE_RN(6);
CALL PREDICT_CUSTOMER_CHURN_RN('OEM');
CALL PREDICT_DESIGN_WIN_CONVERSION_RN('PIC');
*/
