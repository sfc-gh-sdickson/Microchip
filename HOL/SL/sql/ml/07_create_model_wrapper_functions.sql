...
DROP FUNCTION IF EXISTS PREDICT_REVENUE_SL(INT);

CREATE OR REPLACE PROCEDURE PREDICT_REVENUE_SL(
    MONTHS_AHEAD INT
)
RETURNS STRING
LANGUAGE PYTHON
...
-- Drop if exists (in case it was created as FUNCTION before)
DROP FUNCTION IF EXISTS PREDICT_CUSTOMER_CHURN_SL(STRING);

CREATE OR REPLACE PROCEDURE PREDICT_CUSTOMER_CHURN_SL(
...
-- Drop if exists (in case it was created as FUNCTION before)
DROP FUNCTION IF EXISTS PREDICT_DESIGN_WIN_CONVERSION_SL(STRING);

CREATE OR REPLACE PROCEDURE PREDICT_DESIGN_WIN_CONVERSION_SL(
...
/*
CALL PREDICT_REVENUE_SL(6);
CALL PREDICT_CUSTOMER_CHURN_SL('OEM');
CALL PREDICT_DESIGN_WIN_CONVERSION_SL('PIC');
*/
