...
-- Customer 360 View
-- ============================================================================
CREATE OR REPLACE VIEW V_CUSTOMER_360_SL AS
SELECT
    c.customer_id,
    c.customer_name,
    ...
    c.created_at,
    c.updated_at
FROM RAW_SL.CUSTOMERS_SL c
LEFT JOIN RAW_SL.DESIGN_ENGINEERS_SL e ON c.customer_id = e.customer_id AND e.engineer_status = 'ACTIVE'
LEFT JOIN RAW_SL.SUPPORT_CONTRACTS_SL sc ON c.customer_id = sc.customer_id
LEFT JOIN RAW_SL.ORDERS_SL o ON c.customer_id = o.customer_id
LEFT JOIN RAW_SL.SUPPORT_TICKETS_SL st ON c.customer_id = st.customer_id
LEFT JOIN RAW_SL.DESIGN_WINS_SL dw ON c.customer_id = dw.customer_id
LEFT JOIN RAW_SL.PRODUCTION_ORDERS_SL po ON c.customer_id = po.customer_id
LEFT JOIN RAW_SL.QUALITY_ISSUES_SL qi ON c.customer_id = qi.customer_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_ENGINEER_ANALYTICS_SL AS
SELECT
    e.engineer_id,
    e.customer_id,
    c.customer_name,
    ...
    e.created_at,
    e.updated_at
FROM RAW_SL.DESIGN_ENGINEERS_SL e
JOIN RAW_SL.CUSTOMERS_SL c ON e.customer_id = c.customer_id
LEFT JOIN RAW_SL.DESIGN_WINS_SL dw ON e.engineer_id = dw.engineer_id
LEFT JOIN RAW_SL.PRODUCTION_ORDERS_SL po ON e.engineer_id = po.engineer_id
LEFT JOIN RAW_SL.SUPPORT_TICKETS_SL st ON e.engineer_id = st.engineer_id
LEFT JOIN RAW_SL.CERTIFICATIONS_SL cert ON e.engineer_id = cert.engineer_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_WIN_ANALYTICS_SL AS
SELECT
    dw.design_win_id,
    dw.customer_id,
    c.customer_name,
    ...
    dw.created_at,
    dw.updated_at
FROM RAW_SL.DESIGN_WINS_SL dw
JOIN RAW_SL.CUSTOMERS_SL c ON dw.customer_id = c.customer_id
JOIN RAW_SL.DESIGN_ENGINEERS_SL e ON dw.engineer_id = e.engineer_id
JOIN RAW_SL.PRODUCT_CATALOG_SL p ON dw.product_id = p.product_id
LEFT JOIN RAW_SL.DISTRIBUTORS_SL d ON dw.distributor_id = d.distributor_id
LEFT JOIN RAW_SL.PRODUCTION_ORDERS_SL po ON dw.design_win_id = po.design_win_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_PRODUCT_PERFORMANCE_SL AS
SELECT
    p.product_id,
    p.product_name,
    p.sku,
    ...
    p.created_at,
    p.updated_at
FROM RAW_SL.PRODUCT_CATALOG_SL p
LEFT JOIN RAW_SL.DESIGN_WINS_SL dw ON p.product_id = dw.product_id
LEFT JOIN RAW_SL.ORDERS_SL o ON p.product_id = o.product_id
LEFT JOIN RAW_SL.SUPPORT_TICKETS_SL st ON p.product_id = st.product_id
LEFT JOIN RAW_SL.QUALITY_ISSUES_SL qi ON p.product_id = qi.product_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DISTRIBUTOR_PERFORMANCE_SL AS
SELECT
    d.distributor_id,
    d.distributor_name,
    d.distributor_type,
    ...
    d.created_at,
    d.updated_at
FROM RAW_SL.DISTRIBUTORS_SL d
LEFT JOIN RAW_SL.DESIGN_WINS_SL dw ON d.distributor_id = dw.distributor_id
LEFT JOIN RAW_SL.PRODUCTION_ORDERS_SL po ON d.distributor_id = po.distributor_id
LEFT JOIN RAW_SL.ORDERS_SL o ON d.distributor_id = o.distributor_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_SUPPORT_TICKET_ANALYTICS_SL AS
SELECT
    st.ticket_id,
    st.customer_id,
    c.customer_name,
    ...
    st.created_at,
    st.updated_at
FROM RAW_SL.SUPPORT_TICKETS_SL st
JOIN RAW_SL.CUSTOMERS_SL c ON st.customer_id = c.customer_id
LEFT JOIN RAW_SL.DESIGN_ENGINEERS_SL e ON st.engineer_id = e.engineer_id
LEFT JOIN RAW_SL.SUPPORT_ENGINEERS_SL se ON st.assigned_engineer_id = se.support_engineer_id
LEFT JOIN RAW_SL.PRODUCT_CATALOG_SL p ON st.product_id = p.product_id;

-- ============================================================================
-- Quality Issue Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_QUALITY_ISSUE_ANALYTICS_SL AS
SELECT
    qi.quality_issue_id,
    qi.customer_id,
    c.customer_name,
    ...
    qi.created_at,
    qi.updated_at
FROM RAW_SL.QUALITY_ISSUES_SL qi
JOIN RAW_SL.CUSTOMERS_SL c ON qi.customer_id = c.customer_id
LEFT JOIN RAW_SL.PRODUCT_CATALOG_SL p ON qi.product_id = p.product_id
LEFT JOIN RAW_SL.DESIGN_ENGINEERS_SL e ON qi.reported_by_engineer_id = e.engineer_id;

-- ============================================================================
-- Revenue Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_REVENUE_ANALYTICS_SL AS
SELECT
    o.order_id,
    o.customer_id,
    c.customer_name,
    ...
    o.created_at,
    o.updated_at
FROM RAW_SL.ORDERS_SL o
JOIN RAW_SL.CUSTOMERS_SL c ON o.customer_id = c.customer_id
LEFT JOIN RAW_SL.PRODUCT_CATALOG_SL p ON o.product_id = p.product_id
LEFT JOIN RAW_SL.DISTRIBUTORS_SL d ON o.distributor_id = d.distributor_id;

-- ============================================================================
-- Certification Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_CERTIFICATION_ANALYTICS_SL AS
SELECT
    ...
    cert.created_at,
    cert.updated_at
FROM RAW_SL.CERTIFICATIONS_SL cert
JOIN RAW_SL.DESIGN_ENGINEERS_SL e ON cert.engineer_id = e.engineer_id
JOIN RAW_SL.CUSTOMERS_SL c ON cert.customer_id = c.customer_id
LEFT JOIN RAW_SL.CERTIFICATION_VERIFICATIONS_SL v ON cert.certification_id = v.certification_id
GROUP BY
