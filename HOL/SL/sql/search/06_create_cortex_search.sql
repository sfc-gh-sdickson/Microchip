...
-- ============================================================================
CREATE OR REPLACE TABLE SUPPORT_TRANSCRIPTS_SL (
    ...
    FOREIGN KEY (ticket_id) REFERENCES SUPPORT_TICKETS_SL(ticket_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_SL(customer_id),
    FOREIGN KEY (support_engineer_id) REFERENCES SUPPORT_ENGINEERS_SL(support_engineer_id)
);

-- ============================================================================
-- Step 2: Create table for application notes
-- ============================================================================
CREATE OR REPLACE TABLE APPLICATION_NOTES_SL (
...
-- ============================================================================
CREATE OR REPLACE TABLE QUALITY_INVESTIGATION_REPORTS_SL (
    ...
    created_at TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(),
    FOREIGN KEY (quality_issue_id) REFERENCES QUALITY_ISSUES_SL(quality_issue_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_SL(customer_id),
    FOREIGN KEY (product_id) REFERENCES PRODUCT_CATALOG_SL(product_id)
);

-- ============================================================================
-- Step 4: Enable change tracking (required for Cortex Search)
-- ============================================================================
ALTER TABLE SUPPORT_TRANSCRIPTS_SL SET CHANGE_TRACKING = TRUE;
ALTER TABLE APPLICATION_NOTES_SL SET CHANGE_TRACKING = TRUE;
ALTER TABLE QUALITY_INVESTIGATION_REPORTS_SL SET CHANGE_TRACKING = TRUE;

-- ============================================================================
-- Step 5: Generate sample support transcripts
-- ============================================================================
INSERT INTO SUPPORT_TRANSCRIPTS_SL
SELECT
...
LEFT JOIN RAW_SL.PRODUCT_CATALOG_SL pc ON st.product_id = pc.product_id
...
-- ============================================================================
INSERT INTO APPLICATION_NOTES_SL VALUES
...
void enter_standby_mode(void) {
    ...
    SCB->SCR |= SCB_SCR_SLEEPDEEP_Msk;
    PM->SLEEP.reg = PM_SLEEP_IDLE_SLANDBY;
...
ADC->EVCTRL.reg = ADC_EVCTRL_SLARTEI;
```
...
-- ============================================================================
INSERT INTO QUALITY_INVESTIGATION_REPORTS_SL
SELECT
...
JOIN RAW_SL.PRODUCT_CATALOG_SL p ON qi.product_id = p.product_id
...
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE SUPPORT_TRANSCRIPTS_SEARCH_SL
  ON transcript_text
...
AS
  SELECT
    ...
    resolution_provided,
    created_at
  FROM RAW_SL.SUPPORT_TRANSCRIPTS_SL;

-- ============================================================================
-- Step 9: Create Cortex Search Service for Application Notes
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE APPLICATION_NOTES_SEARCH_SL
  ON content
...
AS
  SELECT
    ...
    download_count,
    created_at
  FROM RAW_SL.APPLICATION_NOTES_SL;

-- ============================================================================
-- Step 10: Create Cortex Search Service for Quality Investigation Reports
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE QUALITY_INVESTIGATION_REPORTS_SEARCH_SL
  ON report_text
...
AS
  SELECT
    ...
    investigated_by,
    created_at
  FROM RAW_SL.QUALITY_INVESTIGATION_REPORTS_SL;

-- ============================================================================
-- Step 11: Verify Cortex Search Services Created
-- ============================================================================
SHOW CORTEX SEARCH SERVICES IN SCHEMA RAW_SL;
...
FROM (
  SELECT 'SUPPORT_TRANSCRIPTS_SEARCH_SL' AS service_name
  UNION ALL
  SELECT 'APPLICATION_NOTES_SEARCH_SL'
  UNION ALL
  SELECT 'QUALITY_INVESTIGATION_REPORTS_SEARCH_SL'
);

-- ============================================================================
-- Display data counts
-- ============================================================================
SELECT 'SUPPORT_TRANSCRIPTS_SL' AS table_name, COUNT(*) AS row_count FROM SUPPORT_TRANSCRIPTS_SL
UNION ALL
SELECT 'APPLICATION_NOTES_SL', COUNT(*) FROM APPLICATION_NOTES_SL
UNION ALL
SELECT 'QUALITY_INVESTIGATION_REPORTS_SL', COUNT(*) FROM QUALITY_INVESTIGATION_REPORTS_SL
ORDER BY table_name;
