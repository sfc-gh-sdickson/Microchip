...
-- ============================================================================
CREATE OR REPLACE TABLE SUPPORT_TRANSCRIPTS_ST (
    ...
    FOREIGN KEY (ticket_id) REFERENCES SUPPORT_TICKETS_ST(ticket_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_ST(customer_id),
    FOREIGN KEY (support_engineer_id) REFERENCES SUPPORT_ENGINEERS_ST(support_engineer_id)
);

-- ============================================================================
-- Step 2: Create table for application notes
-- ============================================================================
CREATE OR REPLACE TABLE APPLICATION_NOTES_ST (
...
-- ============================================================================
CREATE OR REPLACE TABLE QUALITY_INVESTIGATION_REPORTS_ST (
    ...
    created_at TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(),
    FOREIGN KEY (quality_issue_id) REFERENCES QUALITY_ISSUES_ST(quality_issue_id),
    FOREIGN KEY (customer_id) REFERENCES CUSTOMERS_ST(customer_id),
    FOREIGN KEY (product_id) REFERENCES PRODUCT_CATALOG_ST(product_id)
);

-- ============================================================================
-- Step 4: Enable change tracking (required for Cortex Search)
-- ============================================================================
ALTER TABLE SUPPORT_TRANSCRIPTS_ST SET CHANGE_TRACKING = TRUE;
ALTER TABLE APPLICATION_NOTES_ST SET CHANGE_TRACKING = TRUE;
ALTER TABLE QUALITY_INVESTIGATION_REPORTS_ST SET CHANGE_TRACKING = TRUE;

-- ============================================================================
-- Step 5: Generate sample support transcripts
-- ============================================================================
INSERT INTO SUPPORT_TRANSCRIPTS_ST
SELECT
...
LEFT JOIN RAW_ST.PRODUCT_CATALOG_ST pc ON st.product_id = pc.product_id
...
-- ============================================================================
INSERT INTO APPLICATION_NOTES_ST VALUES
...
-- ============================================================================
INSERT INTO QUALITY_INVESTIGATION_REPORTS_ST
SELECT
...
JOIN RAW_ST.PRODUCT_CATALOG_ST p ON qi.product_id = p.product_id
...
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE SUPPORT_TRANSCRIPTS_SEARCH_ST
  ON transcript_text
...
AS
  SELECT
    ...
    resolution_provided,
    created_at
  FROM RAW_ST.SUPPORT_TRANSCRIPTS_ST;

-- ============================================================================
-- Step 9: Create Cortex Search Service for Application Notes
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE APPLICATION_NOTES_SEARCH_ST
  ON content
...
AS
  SELECT
    ...
    download_count,
    created_at
  FROM RAW_ST.APPLICATION_NOTES_ST;

-- ============================================================================
-- Step 10: Create Cortex Search Service for Quality Investigation Reports
-- ============================================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE QUALITY_INVESTIGATION_REPORTS_SEARCH_ST
  ON report_text
...
AS
  SELECT
    ...
    investigated_by,
    created_at
  FROM RAW_ST.QUALITY_INVESTIGATION_REPORTS_ST;

-- ============================================================================
-- Step 11: Verify Cortex Search Services Created
-- ============================================================================
SHOW CORTEX SEARCH SERVICES IN SCHEMA RAW_ST;
...
FROM (
  SELECT 'SUPPORT_TRANSCRIPTS_SEARCH_ST' AS service_name
  UNION ALL
  SELECT 'APPLICATION_NOTES_SEARCH_ST'
  UNION ALL
  SELECT 'QUALITY_INVESTIGATION_REPORTS_SEARCH_ST'
);

-- ============================================================================
-- Display data counts
-- ============================================================================
SELECT 'SUPPORT_TRANSCRIPTS_ST' AS table_name, COUNT(*) AS row_count FROM SUPPORT_TRANSCRIPTS_ST
UNION ALL
SELECT 'APPLICATION_NOTES_ST', COUNT(*) FROM APPLICATION_NOTES_ST
UNION ALL
SELECT 'QUALITY_INVESTIGATION_REPORTS_ST', COUNT(*) FROM QUALITY_INVESTIGATION_REPORTS_ST
ORDER BY table_name;
