...
-- Customer 360 View
-- ============================================================================
CREATE OR REPLACE VIEW V_CUSTOMER_360_ST AS
SELECT
    c.customer_id,
    c.customer_name,
...
LEFT JOIN RAW_ST.DESIGN_ENGINEERS_ST e ON c.customer_id = e.customer_id AND e.engineer_status = 'ACTIVE'
LEFT JOIN RAW_ST.SUPPORT_CONTRACTS_ST sc ON c.customer_id = sc.customer_id
...
LEFT JOIN RAW_ST.PRODUCTION_ORDERS_ST po ON c.customer_id = po.customer_id
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_ENGINEER_ANALYTICS_ST AS
SELECT
    e.engineer_id,
    e.customer_id,
    c.customer_name,
    ...
    e.created_at,
    e.updated_at
FROM RAW_ST.DESIGN_ENGINEERS_ST e
JOIN RAW_ST.CUSTOMERS_ST c ON e.customer_id = c.customer_id
LEFT JOIN RAW_ST.DESIGN_WINS_ST dw ON e.engineer_id = dw.engineer_id
LEFT JOIN RAW_ST.PRODUCTION_ORDERS_ST po ON e.engineer_id = po.engineer_id
LEFT JOIN RAW_ST.SUPPORT_TICKETS_ST st ON e.engineer_id = st.engineer_id
LEFT JOIN RAW_ST.CERTIFICATIONS_ST cert ON e.engineer_id = cert.engineer_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DESIGN_WIN_ANALYTICS_ST AS
SELECT
    dw.design_win_id,
    dw.customer_id,
    c.customer_name,
...
JOIN RAW_ST.DESIGN_ENGINEERS_ST e ON dw.engineer_id = e.engineer_id
JOIN RAW_ST.PRODUCT_CATALOG_ST p ON dw.product_id = p.product_id
LEFT JOIN RAW_ST.DISTRIBUTORS_ST d ON dw.distributor_id = d.distributor_id
LEFT JOIN RAW_ST.PRODUCTION_ORDERS_ST po ON dw.design_win_id = po.design_win_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_PRODUCT_PERFORMANCE_ST AS
SELECT
    p.product_id,
    p.product_name,
    p.sku,
    ...
    p.created_at,
    p.updated_at
FROM RAW_ST.PRODUCT_CATALOG_ST p
LEFT JOIN RAW_ST.DESIGN_WINS_ST dw ON p.product_id = dw.product_id
LEFT JOIN RAW_ST.ORDERS_ST o ON p.product_id = o.product_id
LEFT JOIN RAW_ST.SUPPORT_TICKETS_ST st ON p.product_id = st.product_id
LEFT JOIN RAW_ST.QUALITY_ISSUES_ST qi ON p.product_id = qi.product_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_DISTRIBUTOR_PERFORMANCE_ST AS
SELECT
    d.distributor_id,
    d.distributor_name,
    d.distributor_type,
    ...
    d.created_at,
    d.updated_at
FROM RAW_ST.DISTRIBUTORS_ST d
LEFT JOIN RAW_ST.DESIGN_WINS_ST dw ON d.distributor_id = dw.distributor_id
LEFT JOIN RAW_ST.PRODUCTION_ORDERS_ST po ON d.distributor_id = po.distributor_id
LEFT JOIN RAW_ST.ORDERS_ST o ON d.distributor_id = o.distributor_id
GROUP BY
...
-- ============================================================================
CREATE OR REPLACE VIEW V_SUPPORT_TICKET_ANALYTICS_ST AS
SELECT
    st.ticket_id,
    st.customer_id,
    c.customer_name,
    ...
    st.created_at,
    st.updated_at
FROM RAW_ST.SUPPORT_TICKETS_ST st
JOIN RAW_ST.CUSTOMERS_ST c ON st.customer_id = c.customer_id
LEFT JOIN RAW_ST.DESIGN_ENGINEERS_ST e ON st.engineer_id = e.engineer_id
LEFT JOIN RAW_ST.SUPPORT_ENGINEERS_ST se ON st.assigned_engineer_id = se.support_engineer_id
LEFT JOIN RAW_ST.PRODUCT_CATALOG_ST p ON st.product_id = p.product_id;

-- ============================================================================
-- Quality Issue Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_QUALITY_ISSUE_ANALYTICS_ST AS
SELECT
    qi.quality_issue_id,
    qi.customer_id,
    c.customer_name,
    ...
    qi.created_at,
    qi.updated_at
FROM RAW_ST.QUALITY_ISSUES_ST qi
JOIN RAW_ST.CUSTOMERS_ST c ON qi.customer_id = c.customer_id
LEFT JOIN RAW_ST.PRODUCT_CATALOG_ST p ON qi.product_id = p.product_id
LEFT JOIN RAW_ST.DESIGN_ENGINEERS_ST e ON qi.reported_by_engineer_id = e.engineer_id;

-- ============================================================================
-- Revenue Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_REVENUE_ANALYTICS_ST AS
SELECT
    o.order_id,
    o.customer_id,
    c.customer_name,
    ...
    o.created_at,
    o.updated_at
FROM RAW_ST.ORDERS_ST o
JOIN RAW_ST.CUSTOMERS_ST c ON o.customer_id = c.customer_id
LEFT JOIN RAW_ST.PRODUCT_CATALOG_ST p ON o.product_id = p.product_id
LEFT JOIN RAW_ST.DISTRIBUTORS_ST d ON o.distributor_id = d.distributor_id;

-- ============================================================================
-- Certification Analytics View
-- ============================================================================
CREATE OR REPLACE VIEW V_CERTIFICATION_ANALYTICS_ST AS
SELECT
    ...
    cert.created_at,
    cert.updated_at
FROM RAW_ST.CERTIFICATIONS_ST cert
JOIN RAW_ST.DESIGN_ENGINEERS_ST e ON cert.engineer_id = e.engineer_id
JOIN RAW_ST.CUSTOMERS_ST c ON cert.customer_id = c.customer_id
LEFT JOIN RAW_ST.CERTIFICATION_VERIFICATIONS_ST v ON cert.certification_id = v.certification_id
GROUP BY
